...
type
 ...
 private
  ...
  var
   ...
   class function ToDt(const dt:string; t:EDtType):TDateTime; static;
 protected
   procedure InternalOpen;  override;
 public
   constructor Create(AOwner: TComponent; const fName: string; Separator: Char; ClearTmp: Boolean); reintroduce;
...
constructor TTimeDepthTxtDataSet.Create(AOwner: TComponent; const fName: string; Separator: Char; ClearTmp: Boolean);
begin
...
class function TTimeDepthTxtDataSet.ToDt(const dt: string; t: EDtType): TDateTime;
begin
  case t of
    etDelphi: Exit(StrToFloat(dt));
    etString: Exit(StrToDateTime(dt));
         ...
         begin
          ...
          try
            ...
            if rd.datetime <= OldTime then
             begin
               var r := MessageDlg(Format('Error At:%d'+#$D#$A+'old %s > then %s',[idx, DateTimeToStr(OldTime), DateTimeToStr(rd.datetime)]),mtError, [mbIgnore, mbCancel],0);
