   ...
   begin
    ...
    try
     ...
     if wres >= DWORD(WAIT_OBJECT_0+cntev) then
      begin
       var s := Format('E:%d —ектор: %x јдрес: %x', [wres, (ssdFrom+Res.NumLoad) div FStrmSD.FSectorSize, ssdFrom+Res.NumLoad]);
       for var erSec in CheckErrorSectors() do s := s + ' '+ erSec.ToString;
       if Assigned(TDebug.ExeptionEvent) then TDebug.ExeptionEvent('ќшибочный сектор', s, '');
      end;
    finally
     DestroyRead
    end;
