    ...
    if GetLastError <> ERROR_SUCCESS then
     begin
      if NumREad = M32 then
       ...
       begin
        if Assigned(TDebug.ExeptionEvent) then TDebug.ExeptionEvent('ќшибочный сектор',
        Format('—ектор: %x јдрес: %x', [(FOffset+FReadCount + Result.NumLoad) div FStrmSD.FSectorSize, FOffset+FReadCount + Result.NumLoad]), '');
        nread := NumREad;
